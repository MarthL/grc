...
const Corpus = (props) => {
  ...
  const [input, setInput] = useState({label: '', id: null});
  ...
  const handleInputChange = (event, newValue) => {
    setInput({label: event.target.value, id: null});
  };
      ...
      <Box sx={{ my: '5vh', mx: 'auto', width: 200 }}>
        <Autocomplete
          ...
          onChange={(event, newValue) => {
            ...
            setInput({label: newValue.label, id: newValue.id})
          }}
          renderInput={(params) => (
            <TextField
              {...params}
